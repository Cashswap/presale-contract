...
/**
  Allows a decentralised presale to take place, and on success creates a cashswap pair and locks liquidity on Cashsafe.
...
interface IPresaleLockForwarder {
    ...
    function cashswapPairIsInitialised (address _token0, address _token1) external view returns (bool);
}
...
contract Presale01 is ReentrancyGuard {
  ...
  struct PresaleInfo {
    ...
    uint256 LISTING_RATE; // fixed rate at which the token will list on cashswap
    uint256 START_BLOCK;
    uint256 END_BLOCK;
  ...
  struct PresaleFeeInfo {
    uint256 CASHSAFE_BASE_FEE; // divided by 1000
    uint256 CASHSAFE_TOKEN_FEE; // divided by 1000
  ...
  address CASHSAFE_DEV_ADDRESS;
  ICashswapFactory public CASH_FACTORY;
  IWETH public WETH;
  ...
  constructor(address _presaleGenerator) public {
    ...
    CASH_FACTORY = ICashswapFactory(0xCC2FF6957da8E33d19472150301b7D65ACc83f5C);
    ...
    PRESALE_LOCK_FORWARDER = IPresaleLockForwarder(0xf00607CAb96aA2F93256178861779F54272A853d);
    CASHSAFE_DEV_ADDRESS = 0x2854fd5E2FcE375D931b3a17FB53d70d2b681BaE;
  }
  ...
  function init2 (
    ...
    uint256 _cashsafeBaseFee,
    uint256 _cashsafeTokenFee,
    ...
    address payable _referralAddress
      ...
      require(msg.sender == PRESALE_GENERATOR, 'FORBIDDEN');
      // require(!PRESALE_LOCK_FORWARDER.cashswapPairIsInitialised(address(_presaleToken), address(_baseToken)), 'PAIR INITIALISED');
      ...
      PRESALE_FEE_INFO.CASHSAFE_BASE_FEE = _cashsafeBaseFee;
      PRESALE_FEE_INFO.CASHSAFE_TOKEN_FEE = _cashsafeTokenFee;
  ...
  function userDeposit (uint256 _amount) external payable nonReentrant {
    ...
    bool userHoldsCashsafeTokens = PRESALE_SETTINGS.userHoldsSufficientRound1Token(msg.sender);
    if (block.number < PRESALE_INFO.START_BLOCK + STATUS.ROUND1_LENGTH) { // 276 blocks = 1 hour
        require(userHoldsCashsafeTokens, 'INSUFFICENT ROUND 1 TOKEN BALANCE');
    } else if (STATUS.WHITELIST_ONLY && !userHoldsCashsafeTokens) {
      // Round 2
  ...
  // If the pair already exists on cashswap and it contains the presale token as liquidity 
  ...
  function forceFailIfPairExists () external {
    ...
    if (PRESALE_LOCK_FORWARDER.cashswapPairIsInitialised(address(PRESALE_INFO.S_TOKEN), address(PRESALE_INFO.B_TOKEN))) {
        STATUS.FORCE_FAILED = true;
    }
  }
  
  // if something goes wrong in LP generation
  function forceFailByCashsafe () external {
      require(msg.sender == CASHSAFE_DEV_ADDRESS);
      STATUS.FORCE_FAILED = true;
  }
  ...
  function addLiquidity() external nonReentrant {
    ...
    uint256 cashsafeBaseFee = STATUS.TOTAL_BASE_COLLECTED.mul(PRESALE_FEE_INFO.CASHSAFE_BASE_FEE).div(1000);
    
    // base token liquidity
    uint256 baseLiquidity = STATUS.TOTAL_BASE_COLLECTED.sub(cashsafeBaseFee).mul(PRESALE_INFO.LIQUIDITY_PERCENT).div(1000);
    if (PRESALE_INFO.PRESALE_IN_ETH) {
    ...
    // transfer fees
    uint256 cashsafeTokenFee = STATUS.TOTAL_TOKENS_SOLD.mul(PRESALE_FEE_INFO.CASHSAFE_TOKEN_FEE).div(1000);
    // referrals are checked for validity in the presale generator
    if (PRESALE_FEE_INFO.REFERRAL_FEE_ADDRESS != address(0)) {
        // Base token fee
        uint256 referralBaseFee = cashsafeBaseFee.mul(PRESALE_FEE_INFO.REFERRAL_FEE).div(1000);
        ...
        cashsafeBaseFee = cashsafeBaseFee.sub(referralBaseFee);
        // Token fee
        uint256 referralTokenFee = cashsafeTokenFee.mul(PRESALE_FEE_INFO.REFERRAL_FEE).div(1000);
        TransferHelper.safeTransfer(address(PRESALE_INFO.S_TOKEN), PRESALE_FEE_INFO.REFERRAL_FEE_ADDRESS, referralTokenFee);
        cashsafeTokenFee = cashsafeTokenFee.sub(referralTokenFee);
    }
    TransferHelper.safeTransferBaseToken(address(PRESALE_INFO.B_TOKEN), PRESALE_FEE_INFO.BASE_FEE_ADDRESS, cashsafeBaseFee, !PRESALE_INFO.PRESALE_IN_ETH);
    TransferHelper.safeTransfer(address(PRESALE_INFO.S_TOKEN), PRESALE_FEE_INFO.TOKEN_FEE_ADDRESS, cashsafeTokenFee);
